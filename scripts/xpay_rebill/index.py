...
if config['General'].getboolean(f'run_{script_name}'):
    ...
    def generate_dashboard_jinja(campaign_data_1,  client_id):
        ...
        with open('scripts/xpay_rebill/html/campaign-dashboard-template.html', 'r') as file:
        ...
        output_folder = "scripts/xpay_rebill/html/output"  # Define the folder path
            ...
            for report_id in reports_df['id']:
                ...
                if page_df is not None:
                    for page_id in page_df['id']:
                        try:
                            # Execute SQL query to get xpay rebill campaign metrics directly from database
                            xpay_rebill_query = f"""
                            WITH xpay_rebills AS (
                            ...
                            # Execute the query
                            df = fetch_data_from_db(xpay_rebill_query)
                            
                            if df is not None and not df.empty:
                                print(f"Xpay rebill campaign metrics for client {client_id}:")
                                print(df)
                                
                                # Process the SQL query results directly
                                campaign_data_1 = process_sql_campaign_data(df, client_id)
                            else:
                                print(f"No rebill campaign data found for client {client_id}")
                        ...
                        # }
                            ...
                            png_path = os.path.join(image_folder, f"xpay_rebill_{client_id}.png")
                            ...
                            metrics = {
                                ...
                                "last_daily_update_xpay_rebill": str(int(datetime.now().timestamp()))
                            }
    ...
    html_directory = 'scripts/xpay_rebill/html/output'
