...
if config['General'].getboolean(f'run_{script_name}'):
    ...
    def generate_and_save_image(client_metrics, client_id, logo_path):
        ...
        metrics_data = [
            ...
            ("Decline Salvage", "decline_salvage_count","decline_salvage_approval", "decline_salvage_revenue"),
        ...
        for metric_name, count_key, approval_key, revenue_key in metrics_data:
            ...
            if metric_name == "Net" or metric_name == "Gross":
                count_text = f"{count_value:,}" if count_value and isinstance(count_value, (int, float)) else ""
                approval_text = f"{approval_value}%" if approval_value else ""
            else:
                # Handle string values (like decline_salvage_approval) vs numeric values
                if isinstance(count_value, (int, float)):
                    count_text = f"{count_value:,}" if count_value else "0"
                else:
                    count_text = str(count_value) if count_value else "0"
                approval_text = f"{approval_value}%" if approval_value else "0"
                
            # Ensure revenue_value is numeric before applying the format (remove decimals)
            if isinstance(revenue_value, (int, float)) and revenue_value is not None:
                revenue_text = f"${int(revenue_value):,}"
            elif revenue_value is not None and str(revenue_value) != "0":
                # Convert string revenue values to int to remove decimals
                try:
                    revenue_text = f"${int(float(revenue_value)):,}"
                except (ValueError, TypeError):
                    revenue_text = f"${revenue_value}"
            else:
                revenue_text = "$0"
            ...
            name_color = (105, 105, 105) if metric_name in ["Initials", "Rebills", "Decline Salvage", "Straight sales", "Chargebacks", "CS Refunds", "Alert Refunds"] else (0, 0, 0)
            name_font = subtitle_font if metric_name in ["Gross", "Net"] else regular_font  # Use bold font for "Gross" and "Net"
            
            revenue_color = None
            if metric_name in ["Initials", "Rebills", "Decline Salvage", "Straight sales", "Gross", "Net"]:
    ...
    # Function to fetch decline salvage and rebills data using SQL queries
    def fetch_decline_salvage_data(client_id, yesterday):
        try:
            # Query for decline salvage (specific gateway_ids)
            decline_salvage_query = f"""
            SELECT 
                COALESCE(SUM(approvals), 0) as decline_salvage_count,
                COALESCE(ROUND(SUM(revenue)), 0) as decline_salvage_revenue,
                CASE 
                    WHEN SUM(attempts) > 0 THEN ROUND((SUM(approvals_organic)::decimal / SUM(attempts)) * 100, 1)
                    ELSE 0 
                END as decline_salvage_approval
            FROM reporting.order_summary_{client_id} 
            WHERE date = '{yesterday}' 
            AND sales_type = 'Rebills' 
            AND gateway_id IN (1598, 1592, 1570, 1564, 1555);
            """
            
            # Query for regular rebills (excluding specific gateway_ids)
            regular_rebills_query = f"""
            SELECT 
                COALESCE(SUM(approvals), 0) as regular_rebills_count,
                COALESCE(ROUND(SUM(revenue)), 0) as regular_rebills_revenue,
                CASE 
                    WHEN SUM(attempts) > 0 THEN ROUND((SUM(approvals_organic)::decimal / SUM(attempts)) * 100, 1)
                    ELSE 0 
                END as regular_rebills_approval
            FROM reporting.order_summary_{client_id} 
            WHERE date = '{yesterday}' 
            AND sales_type = 'Rebills' 
            AND gateway_id NOT IN (1598, 1592, 1570, 1564, 1555);
            """
            
            # Execute both queries
            decline_salvage_df = fetch_data_from_db(decline_salvage_query)
            regular_rebills_df = fetch_data_from_db(regular_rebills_query)
            
            if decline_salvage_df is not None and regular_rebills_df is not None:
                decline_salvage_row = decline_salvage_df.iloc[0]
                regular_rebills_row = regular_rebills_df.iloc[0]
                
                return {
                    'decline_salvage_count': int(decline_salvage_row['decline_salvage_count']) if decline_salvage_row['decline_salvage_count'] else 0,
                    'decline_salvage_approval': str(decline_salvage_row['decline_salvage_approval']) if decline_salvage_row['decline_salvage_approval'] else '0.0',
                    'decline_salvage_revenue': int(decline_salvage_row['decline_salvage_revenue']) if decline_salvage_row['decline_salvage_revenue'] else 0,
                    'regular_rebills_count': int(regular_rebills_row['regular_rebills_count']) if regular_rebills_row['regular_rebills_count'] else 0,
                    'regular_rebills_approval': str(regular_rebills_row['regular_rebills_approval']) if regular_rebills_row['regular_rebills_approval'] else '0.0',
                    'regular_rebills_revenue': int(regular_rebills_row['regular_rebills_revenue']) if regular_rebills_row['regular_rebills_revenue'] else 0
                }
            
            return None

        except Exception as e:
            print(f"Error fetching decline salvage data for client {client_id}: {e}")
            return None
    ...
    try:
        ...
        client_ids = ['10005']  # Can be expanded to more clients later
        
        for client_id in client_ids:
            print(f"\nProcessing client {client_id}...")
            
            # Build main query for individual client using the new structure
            # Note: Rebills are calculated separately in fetch_decline_salvage_data() to split them properly
            client_query = f"""
            SELECT
              ...
              COALESCE(ROUND(SUM(CASE WHEN sales_type = 'Initials' THEN revenue END)), 0) AS initials_revenue,

              COALESCE(SUM(CASE WHEN sales_type = 'Straight Sales' THEN approvals END), 0) AS straight_sales,
              COALESCE(ROUND(SUM(CASE WHEN sales_type = 'Straight Sales' THEN revenue END)), 0) AS straight_sales_revenue,

              -- gross revenue (total)
              COALESCE(ROUND(SUM(revenue)), 0) AS gross_revenue,

              -- chargebacks
              COALESCE(SUM(cb_cb_date), 0) AS chargebacks_count,
              COALESCE(ROUND(SUM(cb_cb_date_dollar)), 0) AS chargebacks,
              ...
              COALESCE(ROUND(SUM(CASE WHEN refund_type = 'Refund Alert' THEN refund_refund_date_dollar END)), 0) AS refund_alert_amount,

              COALESCE(SUM(CASE WHEN refund_type = 'Refund CS' THEN refund_refund_date END), 0) AS refund_cs_count,
              COALESCE(ROUND(SUM(CASE WHEN refund_type = 'Refund CS' THEN refund_refund_date_dollar END)), 0) AS refund_cs_amount,
            ...
            try:
                ...
                if metrics_df is not None and not metrics_df.empty:
                    ...
                    # Fetch decline salvage and regular rebills data using SQL queries
                    rebills_data = fetch_decline_salvage_data(db_id, yesterday)
                    
                    # Set default values if data is not available
                    decline_salvage_count = 0
                    decline_salvage_approval = "0.0"
                    decline_salvage_revenue = 0
                    regular_rebills_count = 0
                    regular_rebills_approval = "0.0"
                    regular_rebills_revenue = 0
                    
                    if rebills_data:
                        decline_salvage_count = rebills_data['decline_salvage_count']
                        decline_salvage_approval = rebills_data['decline_salvage_approval']
                        decline_salvage_revenue = rebills_data['decline_salvage_revenue']
                        regular_rebills_count = rebills_data['regular_rebills_count']
                        regular_rebills_approval = rebills_data['regular_rebills_approval']
                        regular_rebills_revenue = rebills_data['regular_rebills_revenue']
                    ...
                    # Use regular rebills approval from SQL data (already calculated separately)
                    rebills_approval = regular_rebills_approval
                    ...
                    metrics = {
                        ...
                        "rebills": regular_rebills_count,  # Use regular rebills from SQL data
                        "rebills_revenue": regular_rebills_revenue,  # Use regular rebills revenue from SQL data
                        "decline_salvage_count": decline_salvage_count,  # Add decline salvage count
                        "decline_salvage_approval": decline_salvage_approval,  # Add decline salvage approval rate
                        "decline_salvage_revenue": decline_salvage_revenue,  # Add decline salvage revenue
                        ...
                        "refund_alert_count": row['refund_alert_count'],
                        "refund_cs_count": row['refund_cs_count'],
                        "refund_alert_amount": row['refund_alert_amount'],
                    ...
                    for key, value in metrics.items():
                        if isinstance(value, (Decimal, float)) and key not in ["initials_approval", "rebills_approval", "ss_approval", "decline_salvage_approval"]:
