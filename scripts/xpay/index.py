            ...
            for report_id in reports_df['id']:
                ...
                if page_df is not None:
                    for page_id in page_df['id']:
                        try:
                            # Execute SQL query to get xpay campaign metrics directly from database
                            xpay_query = f"""
                            WITH xpay_initials AS (
                                SELECT client_id, campaign_id, date, sales_type, attempts_total, approvals, revenue
                            ...
                            # Execute the query
                            df = fetch_data_from_db(xpay_query)
                            
                            if df is not None and not df.empty:
                                print(f"Xpay campaign metrics for client {client_id}:")
                                print(df)
                                
                                # Process the SQL query results directly
                                campaign_data_1 = process_sql_campaign_data(df, client_id)
                            else:
                                print(f"No campaign data found for client {client_id}")
                        ...
                        #     print("Execution completed successfully, removed files from images directory")
                            ...
                            print(f"Error fetching xpay metrics for client {client_id}: {e}")
                            summary.add_failure(
