...
if config['General'].getboolean(f'run_{script_name}'):
    ...
    def generate_dashboard_jinja(campaign_data_1, campaign_data_2, client_id):
        ...
        and not (campaign_data_2 and campaign_data_2.get("Metric") and campaign_data_2.get("Value")):
            print("No campaign data to display. Skipping HTML generation.")
            return None  # or raise an exception, or handle as needed
        
        with open('scripts/xpay/html/campaign-dashboard-template.html', 'r') as file:
            template = Template(file.read())
        
        html_content = template.render(
            campaign_data_1=json.dumps(campaign_data_1 or {}),
            campaign_data_2=json.dumps(campaign_data_2 or {})
        )
        
        output_folder = "scripts/xpay/html/output"  # Define the folder path
                ...
                if page_df is not None:
                    for page_id in page_df['id']:
                                ...
                                ADDCOLUMNS (
                                    SUMMARIZE (
                                        ADDCOLUMNS (
                                            FILTER (
                                                order_details,
                                                order_details[date_of_sale] >= TODAY() - 28 &&
                                                ...
                                                (
                                                    SEARCH("Step 1", order_details[Campaign name], 1, 0) > 0 ||
                                                    SEARCH("Step1", order_details[Campaign name], 1, 0) > 0
                                                )
                                            ),
                        ...
                        try:
                            ...
                            output_file = generate_dashboard_jinja(campaign_data_1, campaign_data_2, client_id)
                            if output_file:
                            ...
                            png_path = os.path.join(image_folder, f"xpay_{client_id}.png")
                            ...
                            metrics = {
                                ...
                                "last_daily_update_xpay": str(int(datetime.now().timestamp()))
                            }
