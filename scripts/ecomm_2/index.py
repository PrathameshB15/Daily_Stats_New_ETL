...
if config['General'].getboolean(f'run_{script_name}'):
    ...
    def process_campaign_data(url, headers, payload, client_id):
        ...
        # Debug: Print actual column names to understand the structure
        print(f"Actual columns in DataFrame: {df.columns.tolist()}")
        print(f"Sample data: {df.head()}")

        # 1) Handle column renaming more robustly
        # Check what columns actually exist and rename them appropriately
        column_mapping = {}
        for col in df.columns:
            if 'product_price' in col.lower():
                column_mapping[col] = 'ProductPrice'
            elif 'rebill' in col.lower() and 'approval' in col.lower():
                column_mapping[col] = 'RebillApprovals'
            elif 'order' in col.lower() and 'total' in col.lower():
                column_mapping[col] = 'OrderTotal'

        df = df.rename(columns=column_mapping)
        
        # If we still don't have the expected columns, try to identify them by position
        if 'ProductPrice' not in df.columns and len(df.columns) >= 1:
            df = df.rename(columns={df.columns[0]: 'ProductPrice'})
        if 'RebillApprovals' not in df.columns and len(df.columns) >= 2:
            df = df.rename(columns={df.columns[1]: 'RebillApprovals'})
        if 'OrderTotal' not in df.columns and len(df.columns) >= 3:
            df = df.rename(columns={df.columns[2]: 'OrderTotal'})

        # 2) Make RebillApprovals numeric (handles any formatting issues)
        if 'RebillApprovals' in df.columns:
            df['RebillApprovals'] = (
                df['RebillApprovals']
                .astype(str)
            ...
            df['RebillApprovals'] = 0

        # 3) Make ProductPrice numeric and format it properly
        ...
        # 4) Make OrderTotal numeric and format it properly
        if 'OrderTotal' in df.columns:
            df['OrderTotal'] = (
                df['OrderTotal']
                .astype(str)
                .str.replace(r'[\$,]', '', regex=True)   # drop $ and commas if present
        ...
        # Process each product price, rebill approvals, and order total
        for i in range(len(df)):
            product = df.iloc[i]
            product_price = f"${product['ProductPrice']:,.2f}" if pd.notna(product['ProductPrice']) else "N/A"
            rebill_approvals = str(int(product['RebillApprovals'])) if pd.notna(product['RebillApprovals']) else "0"
            order_total = f"${product['OrderTotal']:,.0f}" if pd.notna(product['OrderTotal']) else "$0"

            campaign_data["Metric"].extend([
                f"Product {i+1} Price", 
                f"Product {i+1} Rebill Approvals",
                f"Product {i+1} Order Total"
            ])
    ...
    def generate_dashboard_jinja(campaign_data_1,  client_id):
        ...
        with open('scripts/ecomm_2/html/campaign-dashboard-template.html', 'r') as file:
        ...
        output_folder = "scripts/ecomm_2/html/output"  # Define the folder path
                ...
                if page_df is not None:
                    for page_id in page_df['id']:
                        ...
                        try:
                            ...
                            png_path = os.path.join(image_folder, f"custom_{client_id}_2.png")
                            ...
                            metrics = {
                                ...
                                "last_daily_update_custom_2": str(int(datetime.now().timestamp()))
                            }
    ...
    html_directory = 'scripts/ecomm_2/html/output'
