...
if config['General'].getboolean(f'run_{script_name}'):
    ...
    def generate_and_save_image(client_metrics, client_id, logo_path):
        ...
        logo_y = 40  # Y position for both logo and title (increased space from top)
        ...
        subtitle_y = logo_y + logo.height - 10 # Position the subtitle below the title
        ...
        metrics_data = [
            ("PP Initials", "pp_initials","pp_initials_approval", "pp_initials_revenue"),
            ("Non PP Initials", "non_pp_initials","non_pp_initials_approval", "non_pp_initials_revenue"),
            ("Rebills", "rebills","rebills_approval", "rebills_revenue"),
            ("Straight Sales", "ss","ss_approval", "ss_revenue"),
        ...
        for metric_name, count_key, approval_key, revenue_key in metrics_data:
            ...
            name_color = (105, 105, 105) if metric_name in ["PP Initials", "Non PP Initials", "Straight Sales", "Rebills", "Chargebacks", "Refund", "CS Refunds", "Alert Refunds"] else (0, 0, 0)
            name_font = subtitle_font if metric_name in ["Gross", "Net"] else regular_font  # Use bold font for "Gross" and "Net"
            
            revenue_color = None
            if metric_name in ["PP Initials", "Non PP Initials", "Straight Sales", "Rebills", "Gross", "Net"]:
    ...
    client_query = "select client_id from beast_insights_v2.crm_credentials where client_id = '10041';"
    ...
    if clients_df is not None:
        ...
        for client_id in clients_df['client_id']:
                ...
                for report_id in reports_df['id']:
                    ...
                    if page_df is not None:
                        for page_id in page_df['id']:
                            ...
                            cookies = login_and_extract_cookies("dev_10041", "Dev@beastinsights/#2024")
                            ...
                            payload = {
                                ...
                                UNION(
                                    CALCULATETABLE( 
                                        ROW(
                                            ...
                                            CALCULATE(
                                                ...
                                                order_details[istest] = "No",
                                                campaigns[Campaign Type] = "Prepaid"
                                                
                                            )
                                        )
                                    ),
                                    ROW(
                                        "Prepaid Initials Approval Rate (%)", 
                                        FORMAT(
                                            CALCULATE(
                                    ...
                                    ROW(
                                        "Non Prepaid Initials Approval Rate (%)", 
                                        FORMAT(
                                            CALCULATE(
                                                DIVIDE([# Approvals], [# Attempts]), 
                                                order_details[is_test] = "No", 
                                    ...
                                    ROW(
                                        "Non Prepaid Initials Total Order Value", 
                                        CALCULATE(
                                            SUM(order_details[order_total]), 
                                            order_details[is_test] = "No", 
                                            Calendar[Date] = TODAY() - 1, 
                            ...
                            try:
                                ...
                                df = pd.DataFrame({
                                    'Initials': ['Prepaid Initials', 'Non Prepaid Initials'],
                                ...
                                pp_initials_count = df.loc[df['Initials'] == 'Prepaid Initials', 'Count'].values[0]
                                pp_initials_approval = df.loc[df['Initials'] == 'Prepaid Initials', 'Approval %'].values[0]
                                if pp_initials_approval == '':
                                    pp_initials_approval = '0.0%'
                                pp_initials_value = df.loc[df['Initials'] == 'Prepaid Initials', 'Value'].values[0]

                                non_pp_initials_count = df.loc[df['Initials'] == 'Non Prepaid Initials', 'Count'].values[0]
                                non_pp_initials_approval = df.loc[df['Initials'] == 'Non Prepaid Initials', 'Approval %'].values[0]
                                if non_pp_initials_approval == '':
                                    non_pp_initials_approval = '0.0%'
                                non_pp_initials_value = df.loc[df['Initials'] == 'Non Prepaid Initials', 'Value'].values[0]

                                
                                # Print extracted variables
                                print(f"PP Initials -> Count: {pp_initials_count}, Approval %: {pp_initials_approval}, Value: {pp_initials_value}")
                                print(f"Non PP Initials -> Count: {non_pp_initials_count}, Approval %: {non_pp_initials_approval}, Value: {non_pp_initials_value}")
            ...
            try:
                db_metrics_query = f"SELECT rebills, rebills_revenue, straight_sales, straight_sales_revenue, rebills_organic_approvals, rebills_organic_attempts, straights_sales_organic_approvals, straights_sales_organic_attempts, gross_revenue, net_revenue, chargebacks_count, chargebacks, refund_count, refund, refund_alert_count, refund_cs_count, refund_alert_amount, refund_cs_amount FROM beast_insights_v2.daily_data_sync where client_id = {client_id} and date = CURRENT_DATE - INTERVAL '1 day';"          
                db_metrics = fetch_data_from_db(db_metrics_query) 
                print(db_metrics)   
                # Ensure df_metrics exists
                if not db_metrics.empty:
                    rebills = db_metrics.loc[0, 'rebills']
                    ss = db_metrics.loc[0, 'straight_sales']
                    rebills_revenue = db_metrics.loc[0, 'rebills_revenue']
                    ss_revenue = db_metrics.loc[0, 'straight_sales_revenue']
                    rebills_organic_approvals = db_metrics.loc[0, 'rebills_organic_approvals']
                    rebills_organic_attempts = db_metrics.loc[0, 'rebills_organic_attempts']
                    straights_sales_organic_approvals = db_metrics.loc[0, 'straights_sales_organic_approvals']
                    straights_sales_organic_attempts = db_metrics.loc[0, 'straights_sales_organic_attempts']
                    ...
                    rebills_approval = (
                        f"{(rebills_organic_approvals / rebills_organic_attempts) * 100:.1f}"
                        if rebills_organic_approvals is not None and rebills_organic_attempts not in (None, 0)
                        else "0.00"
                    )

                    ss_approval = (
                        f"{(straights_sales_organic_approvals / straights_sales_organic_attempts) * 100:.1f}"
                        if straights_sales_organic_attempts not in (None, 0) and straights_sales_organic_approvals is not None
                        else "0.00"
                    )
                    ...
                    chargebacks_count = chargebacks = refund_count = refund = net_revenue = gross_revenue = rebills = ss = ss_revenue = rebills_revenue = rebills_approval = ss_approval = refund_cs_count = refund_cs_amount = refund_alert_count = refund_alert_amount = None
                ...
                rebills = round(rebills) if rebills is not None else 0
                ss = round(ss) if ss is not None else 0
                ss_revenue = round(ss_revenue) if ss_revenue is not None else 0
                rebills_revenue = round(rebills_revenue) if rebills_revenue is not None else 0
                rebills_approval = round(float(rebills_approval), 2) if rebills_approval is not None else 0
                ss_approval = round(float(ss_approval), 2) if ss_approval is not None else 0
                ...
                rebills = f"{rebills:,}" if rebills is not None else 0
                ss = f"{ss:,}" if ss is not None else 0
                ss_revenue = f"{ss_revenue:,}" if ss_revenue is not None else 0
                rebills_revenue = f"{rebills_revenue:,}" if rebills_revenue is not None else 0
                rebills_approval = f"{rebills_approval:,}" if rebills_approval is not None else 0
                ss_approval = f"{ss_approval:,}" if ss_approval is not None else 0
                ...
                rebills = str(rebills)
                ss = str(ss)
                ss_revenue = str(ss_revenue)
                rebills_revenue = str(rebills_revenue)
                rebills_approval = str(rebills_approval) + "%"
                ss_approval = str(ss_approval) + "%"
                ...
                print(f"rebills: {rebills}")
                print(f"ss: {ss}")
                print(f"ss_revenue: {ss_revenue}")
                print(f"rebills_revenue: {rebills_revenue}")
                print(f"rebills_approval: {rebills_approval}")
                print(f"ss_approval: {ss_approval}")
                ...
                data = {
                    "Metric": [
                        "PP Initials count", "PP Initials Approval %", "PP Initials Value",
                        "Non PP Initials Count", "Non PP Initials Approval %", "Non PP Initials Value",
                        "Rebill Count", "Rebill Approval %", "Rebill Value",
                        "SS count", "SS Approval %", "SS value",
                        "Gross", "Net",
                    ...
                    "Value": [
                        pp_initials_count, pp_initials_approval, pp_initials_value,
                        non_pp_initials_count, non_pp_initials_approval, non_pp_initials_value,
                        rebills, rebills_approval, rebills_revenue,
                        ss, ss_approval, ss_revenue,
                ...
                pp_initials_value = round(float(pp_initials_value)) if pp_initials_value is not None else 0 # Convert to float and round
                pp_initials_value = f"{pp_initials_value:,}" if pp_initials_value is not None else 0
                pp_initials_value = str(pp_initials_value) if pp_initials_value is not None else 0  # Convert back to string
                
                non_pp_initials_value = round(float(non_pp_initials_value)) if non_pp_initials_value is not None else 0  # Convert to float and round
                non_pp_initials_value = f"{non_pp_initials_value:,}" if non_pp_initials_value is not None else 0
                non_pp_initials_value = str(non_pp_initials_value) if non_pp_initials_value is not None else 0  # Convert back to string

                pp_initials_count = round(float(pp_initials_count)) if pp_initials_count is not None else 0
                pp_initials_count = f"{pp_initials_count:,}" if pp_initials_count is not None else 0
                pp_initials_count = str(pp_initials_count) if pp_initials_count is not None else 0
                
                non_pp_initials_count = round(float(non_pp_initials_count)) if non_pp_initials_count is not None else 0
                non_pp_initials_count = f"{non_pp_initials_count:,}" if non_pp_initials_count is not None else 0
                non_pp_initials_count = str(non_pp_initials_count) if non_pp_initials_count is not None else 0
                
                
                metrics = {
                    "db_id": client_id,  # Set db_id as the identifier in the metrics payload
                    "pp_initials": pp_initials_count,
                    "pp_initials_revenue": pp_initials_value,
                    "pp_initials_approval": pp_initials_approval,
                    
                    "non_pp_initials": non_pp_initials_count,
                    "non_pp_initials_revenue": non_pp_initials_value,
                    "non_pp_initials_approval": non_pp_initials_approval,
                    
                    "rebills": rebills,
                    "rebills_revenue": rebills_revenue,
