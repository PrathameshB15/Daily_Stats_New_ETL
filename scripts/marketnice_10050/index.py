...
if config['General'].getboolean(f'run_{script_name}'):
    ...
    try:
        # Main script to fetch data and push to Customer.io
        # query = "SELECT client_id, initials, initials_revenue, rebills, rebills_revenue, straight_sales, straight_sales_revenue, gross_revenue, chargebacks_count, chargebacks, refund_count, refund, net_revenue, initials_organic_approvals, initials_organic_attempts, rebills_organic_approvals, rebills_organic_attempts, straights_sales_organic_approvals, straights_sales_organic_attempts, refund_alert_count, refund_cs_count, refund_alert_amount, refund_cs_amount FROM beast_insights_v2.daily_data_sync where date = CURRENT_DATE - INTERVAL '1 day' and client_id NOT IN ('10001', '10002', '10003', '10006', '10015', '10018', '10020', '10021', '10026', '10028', '10027', '20001', '20002', '20003', '20004', '20005', '20006', '20007', '20008', '20009', '20010', '20011', '20012', '20013', '20014', '20015', '20016', '20018', '10025', '10033', '10030', '10041', '10019', '10005');"  # Include id column in the query
        query = "SELECT client_id, initials, initials_revenue, rebills, rebills_revenue, straight_sales, straight_sales_revenue, gross_revenue, chargebacks_count, chargebacks, refund_count, refund, net_revenue, initials_organic_approvals, initials_organic_attempts, rebills_organic_approvals, rebills_organic_attempts, straights_sales_organic_approvals, straights_sales_organic_attempts, refund_alert_count, refund_cs_count, refund_alert_amount, refund_cs_amount FROM beast_insights_v2.daily_data_sync where date = CURRENT_DATE - INTERVAL '1 day' and client_id IN ('10050')"  # Include id column in the query

        # query = "SELECT client_id, initials, initials_revenue, rebills, rebills_revenue, straight_sales, straight_sales_revenue, gross_revenue, chargebacks_count, chargebacks, refund_count, refund, net_revenue FROM beast_insights_v2.daily_data_sync where date = '2024-11-09';"  # Include id column in the query
        # query = "SELECT client_id, initials, initials_revenue, rebills, rebills_revenue, straight_sales, straight_sales_revenue, gross_revenue, chargebacks_count, chargebacks, refund_count, refund, net_revenue, initials_organic_approvals, initials_organic_attempts, rebills_organic_approvals, rebills_organic_attempts, straights_sales_organic_approvals, straights_sales_organic_attempts, refund_alert_count, refund_cs_count, refund_alert_amount, refund_cs_amount FROM beast_insights_v2.daily_data_sync where date = CURRENT_DATE - INTERVAL '1 day' and client_id = '10008';"  # Include id column in the query
        # query = "SELECT client_id, initials, initials_revenue, rebills, rebills_revenue, straight_sales, straight_sales_revenue, gross_revenue, chargebacks_count, chargebacks, refund_count, refund, net_revenue, initials_organic_approvals, initials_organic_attempts, rebills_organic_approvals, rebills_organic_attempts, straights_sales_organic_approvals, straights_sales_organic_attempts  FROM beast_insights_v2.daily_data_sync where date = CURRENT_DATE - INTERVAL '1 day' and client_id = 10001 order by client_id ;"
        metrics_df = fetch_data_from_db(query)
        ...
        if metrics_df is not None:
            ...
            for _, row in metrics_df.iterrows():
                try:
                    db_id = row['client_id']  # Map database id to db_id
                    # Calculate additional metrics
                    # Calculate additional metrics and format with 2 decimal points
                    # initials_approval = (
                    #     f"{(row['initials_organic_approvals'] / row['initials_organic_attempts']) * 100:.1f}"
                    #     if row['initials_organic_attempts'] is not None and row['initials_organic_approvals'] is not None else "0.00"
                    # )
                    # rebills_approval = (
                    #     f"{(row['rebills_organic_approvals'] / row['rebills_organic_attempts']) * 100:.1f}"
                    #     if row['rebills_organic_approvals'] is not None and row['rebills_organic_attempts'] is not None else "0.00"
                    # )
                    # ss_approval = (
                    #     f"{(row['straights_sales_organic_approvals'] / row['straights_sales_organic_attempts']) * 100:.1f}"
                    #     if row['straights_sales_organic_attempts'] is not None and row['straights_sales_organic_approvals'] is not None else "0.00"
                    # )
                    
                    initials_approval = (
                        f"{(row['initials_organic_approvals'] / row['initials_organic_attempts']) * 100:.1f}"
                        if row['initials_organic_approvals'] is not None and row['initials_organic_attempts'] not in (None, 0)
                        else "0.00"
                    )

                    rebills_approval = (
                        f"{(row['rebills_organic_approvals'] / row['rebills_organic_attempts']) * 100:.1f}"
                        if row['rebills_organic_approvals'] is not None and row['rebills_organic_attempts'] not in (None, 0)
                        else "0.00"
                    )

                    ss_approval = (
                        f"{(row['straights_sales_organic_approvals'] / row['straights_sales_organic_attempts']) * 100:.1f}"
                        if row['straights_sales_organic_approvals'] is not None and row['straights_sales_organic_attempts'] not in (None, 0)
                        else "0.00"
                    )
                    ...
                    metrics = {
                        ...
                        # "last_daily_update": datetime.now().strftime('%Y-%m-%d')
                        "last_daily_update_marketnice": int(datetime.now().timestamp())
                        
                    }

             
                    
                    # Update numeric values to whole numbers
                    # for key, value in metrics.items():
                    #     if isinstance(value, Decimal):  # Check if the value is a float
                    #         metrics[key] = int(value)  # Convert to whole number using int (truncates decimals)
                            
                    # for key, value in metrics.items():
                    #     if isinstance(value, int) and key not in ["db_id", "last_daily_update"]:  # Check if the value is a float
                    #         # Convert to int (round off decimals) and then format with commas
                    ...
                    #logo = "D:\\Daily_metrics_customer_io\\3x Icon.png"
            ...
            print("No data to push to Customer.io.")   
