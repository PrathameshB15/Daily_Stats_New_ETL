...
if config['General'].getboolean(f'run_{script_name}'):
    ...
    def generate_dashboard_jinja(campaign_data_1, client_id):
        ...
        with open('scripts/xpay_payfac/html/campaign-dashboard-template.html', 'r') as file:
        ...
        output_folder = "scripts/xpay_payfac/html/output"  # Define the folder path
                ...
                if page_df is not None:
                    for page_id in page_df['id']:
                        try:
                            ...
                            WHERE os.date >= date_trunc('month', CURRENT_DATE)
                            ...
                            # Execute the query
                            df = fetch_data_from_db(payfac_query)
                            
                            if df is not None and not df.empty:
                                print(f"Payfac product metrics for client {client_id}:")
                                print(df)
                                
                                # Process the SQL query results directly
                                campaign_data_1 = process_sql_campaign_data(df, client_id)
                            else:
                                print(f"No payfac product data found for client {client_id}")
    ...
    html_directory = 'scripts/xpay_payfac/html/output'
